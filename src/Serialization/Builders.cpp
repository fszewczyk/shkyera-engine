...
HandleAndAsset<AssetType> fromProto(Registry* registry, const std::map<Entity, Entity>& protoToRegistryEntities,
                                    Entity protoEntity) {
  if (protoEntity == InvalidEntity) {
    return {};
  }

  SHKYERA_ASSERT(protoToRegistryEntities.contains(protoEntity), "Asset {} is not in the registry, so it cannot have {}",
                 protoEntity, typeid(AssetType).name());

  const auto& registryAssetEntity = protoToRegistryEntities.at(protoEntity);
  SHKYERA_ASSERT(registry->hasComponent<AssetComponent<AssetType>>(registryAssetEntity),
                 "Asset {} does not have the Asset Component ({})", protoEntity, typeid(AssetType).name());

  return {registryAssetEntity,
          utils::assets::read(registry->getComponent<AssetComponent<AssetType>>(registryAssetEntity))};
}
...
void loadComponents(Registry* registry, std::map<Entity, Entity>& protoToRegistryEntities,
                    const ProtoArray<ProtoType>& protoComponents, ConversionFunc constructComponent) {
...
template <typename ComponentType, typename ProtoType, typename ConversionFunc>
void loadSingletonComponent(Registry* registry, std::map<Entity, Entity>& protoToRegistryEntities,
                            const ProtoType& protoComponent, ConversionFunc constructComponent) {
  ...
  SHKYERA_ASSERT(!registry->getEntity<ComponentType>().has_value(),
                 "Registry already contains a SingletonComponent {} (Entity: {}, ProtoEntity: {}. Trying to override "
                 "during deserialization is likely wrong",
                 typeid(ComponentType).name(), entity, protoEntity);
...
std::unique_ptr<shkyera::Registry> fromBinary(std::istream& serialized) {
  ...
  loadComponents<AssetComponent<Texture>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.textures_path(),
      [registry = registry.get()](const auto& comp) {
  ...
  loadComponents<AssetComponent<CubeMap>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.cubemaps(),
      [registry = registry.get(), &protoToRegistryEntities](const auto& comp) {
        return AssetComponent<CubeMap>(std::make_unique<utils::assets::CubeMapLoader>(
            registry, std::array<HandleAndAsset<Image>, 6>{
  ...
  loadComponents<AssetComponent<Mesh>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.meshes_path(), [](const auto& comp) {
        return AssetComponent<Mesh>(std::make_unique<utils::assets::PathAssetLoader<Mesh>>(comp.path()));
      });

  loadComponents<AssetComponent<Mesh>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.meshes_factory(), [](const auto& comp) {
        return AssetComponent<Mesh>(
            std::make_unique<utils::assets::FactoryAssetLoader<Mesh>>(Mesh::Factory::Type(comp.type())));
      });

  loadComponents<AssetComponent<Wireframe>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.wireframes_path(), [](const auto& comp) {
        return AssetComponent<Wireframe>(std::make_unique<utils::assets::PathAssetLoader<Wireframe>>(comp.path()));
      });

  loadComponents<AssetComponent<Wireframe>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.wireframes_factory(), [](const auto& comp) {
        return AssetComponent<Wireframe>(
            std::make_unique<utils::assets::FactoryAssetLoader<Wireframe>>(Wireframe::Factory::Type(comp.type())));
      });

  loadComponents<AssetComponent<Material>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.materials(),
      [registry = registry.get(), &protoToRegistryEntities](const auto& comp) {
        ...
        return AssetComponent<Material>(std::make_unique<utils::assets::MaterialLoader>(registry, material));
      });
  ...
  loadSingletonComponent<AssetRoot>(registry.get(), protoToRegistryEntities, serializedRegistry.asset_root(),
                                    [](const auto&) { return AssetRoot{}; });

  // Components
  loadComponents<DirectoryComponent>(registry.get(), protoToRegistryEntities, serializedRegistry.directories(),
                                     [](const auto& comp) { return DirectoryComponent(comp.name()); });

  loadComponents<NameComponent>(registry.get(), protoToRegistryEntities, serializedRegistry.names(),
                                [](const auto& comp) { return NameComponent(comp.name()); });

  loadComponents<TransformComponent>(registry.get(), protoToRegistryEntities, serializedRegistry.transforms(),
                                     [](const auto& comp) {
  ...
  loadComponents<CameraComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.cameras(), [](const auto& comp) {
  ...
  loadComponents<AmbientLightComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.ambient_lights(), [](const auto& comp) {
        return AmbientLightComponent{.intensity = comp.intensity(), .color = fromProto(comp.color())};
      });

  loadComponents<ModelComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.models(),
  ...
  loadComponents<SkyboxComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.skyboxes(),
      [&protoToRegistryEntities, registry = registry.get()](const auto& comp) {
        return SkyboxComponent{.skyboxCubeMap = fromProto<CubeMap>(registry, protoToRegistryEntities, comp.cubemap())};
      });

  loadComponents<BoxColliderComponent<RuntimeMode::DEVELOPMENT>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.box_colliders_dev(), [](const auto& comp) {
  ...
  loadComponents<BoxColliderComponent<RuntimeMode::PRODUCTION>>(
      registry.get(), protoToRegistryEntities, serializedRegistry.box_colliders_prod(), [](const auto& comp) {
        return BoxColliderComponent<RuntimeMode::PRODUCTION>{
            .box = AABB{fromProto(comp.center()), fromProto(comp.extents())}};
      });

  loadComponents<DirectionalLightComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.directional_lights(), [](const auto& comp) {
        return DirectionalLightComponent{.intensity = comp.intensity(), .color = fromProto(comp.color())};
      });

  loadComponents<ParticleEmitterComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.particle_emitters(),
      [registry = registry.get(), &protoToRegistryEntities](const auto& comp) {
  ...
  loadComponents<PointLightComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.point_lights(), [](const auto& comp) {
        return PointLightComponent{
            .intensity = comp.intensity(), .range = comp.range(), .color = fromProto(comp.color())};
      });

  loadComponents<PostProcessingVolumeComponent>(
      registry.get(), protoToRegistryEntities, serializedRegistry.post_processing_volumes(), [](const auto& comp) {
        return PostProcessingVolumeComponent{.global = comp.global(),
                                             .gamma = comp.gamma(),
  ...
  loadComponents<SpotLightComponent>(registry.get(), protoToRegistryEntities, serializedRegistry.spot_lights(),
                                     [](const auto& comp) {
                                       return SpotLightComponent{.intensity = comp.intensity(),
                                                                 .range = comp.range(),
  ...
  loadComponents<WireframeComponent>(registry.get(), protoToRegistryEntities, serializedRegistry.wireframes(),
                                     [&protoToRegistryEntities, registry = registry.get()](const auto& comp) {
                                       WireframeComponent wireframeComponent{};
                                       wireframeComponent.wireframe =
                                           fromProto<Wireframe>(registry, protoToRegistryEntities, comp.wireframe());
  ...
  for (const auto& relationship : serializedRegistry.ancestry_relations()) {
    SHKYERA_ASSERT(protoToRegistryEntities.contains(relationship.parent()),
                   "Parent Proto Entity {} was not loaded. Cannot register a parent-child relationship with {}",
                   relationship.parent(), relationship.child());
    SHKYERA_ASSERT(protoToRegistryEntities.contains(relationship.child()),
                   "Child Proto Entity {} was not loaded. Cannot register a parent-child relationship with {}",
                   relationship.child(), relationship.parent());
    SHKYERA_ASSERT(relationship.child() != relationship.parent(), "Parent and child cannot be equal ({})",
                   relationship.child());
...
void serializeComponents(Registry const* registry, ProtoArray<ProtoType>& protoComponents,
...
void toBinary(std::ostream& outputStream, Registry const* registry) {
  ...
  for (const auto& [entity, mesh] : registry->getComponentSet<AssetComponent<Mesh>>()) {
    ...
    } else if (const auto* meshLoader =
                   dynamic_cast<utils::assets::FactoryAssetLoader<Mesh>*>(mesh.constructionFunction.get())) {
  ...
  serializeComponents<AssetComponent<Material>, shkyeraProto::MaterialAsset>(
      ...
      [](const AssetComponent<Material>& component, shkyeraProto::MaterialAsset& protoComponent) {
        if (const auto* materialLoader =
                dynamic_cast<utils::assets::MaterialLoader*>(component.constructionFunction.get())) {
  ...
  for (const auto& [entity, wireframe] : registry->getComponentSet<AssetComponent<Wireframe>>()) {
    if (const auto* wireframeLoader =
            dynamic_cast<utils::assets::PathAssetLoader<Wireframe>*>(wireframe.constructionFunction.get())) {
    ...
    } else if (const auto* wireframeLoader =
                   dynamic_cast<utils::assets::FactoryAssetLoader<Wireframe>*>(wireframe.constructionFunction.get())) {
  ...
  for (const auto& [entity, image] : registry->getComponentSet<AssetComponent<Image>>()) {
    if (const auto* imageLoader =
            dynamic_cast<utils::assets::PathAssetLoader<Image>*>(image.constructionFunction.get())) {
  ...
  for (const auto& [entity, texture] : registry->getComponentSet<AssetComponent<Texture>>()) {
    if (const auto* textureLoader =
            dynamic_cast<utils::assets::PathAssetLoader<Texture>*>(texture.constructionFunction.get())) {
  ...
  serializeSingletonComponent<AssetRoot>(registry, *serializedRegistry.mutable_asset_root(),
                                         [](const auto&, const auto&) {});

  // Components
  serializeComponents<DirectoryComponent>(
      registry, *serializedRegistry.mutable_directories(),
      [](const auto& component, auto& protoComponent) { protoComponent.set_name(component.name); });
  ...
  serializeComponents<SkyboxComponent>(registry, *serializedRegistry.mutable_skyboxes(),
