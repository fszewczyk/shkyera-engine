...
void ParticleSystem::spawnParticles(ParticleEmitterComponent& emitter, const glm::mat4& transformMatrix)
{
    ...
    auto emissionSampler = getUniformSampler(static_cast<double>(emitter.emittedPerSecond), static_cast<double>(emitter.emittedPerSecondVariance));
    ...
    if(state.accumulatedEmittedParticles >= 1.0)
    {
        ...
        auto initialTransparencySampler = getUniformSampler(emitter.initialTransparency, emitter.initialTransparencyVariance);
