...
RenderingSystem<MainCameraTag>::RenderingSystem(std::shared_ptr<Registry> registry)
    : _registry(registry) {
    SHKYERA_ASSERT(_registry->getEntity<MainCameraTag>().has_value(), "{} CameraTag is not registered. Cannot construct RenderingSystem", typeid(MainCameraTag).name());
    ...
    _particleEmitterDebugMaterial = _directionalLightDebugMaterial = _spotLightDebugMaterial = _pointLightDebugMaterial = _ambientLightDebugMaterial = _postProcessingVolumeDebugMaterial = debugBillboardMaterial;
...
void RenderingSystem<MainCameraTag>::renderViewPosition() {
    ...
    const glm::mat4& viewMatrix = _registry->getComponent<CameraComponent>(*_registry->getEntity<MainCameraTag>()).getViewMatrix(cameraTransform);
    const glm::mat4& projectionMatrix = _registry->getComponent<CameraComponent>(*_registry->getEntity<MainCameraTag>()).getProjectionMatrix();
...
void RenderingSystem<MainCameraTag>::renderWorldObjects(const PostProcessingVolumeComponent& postProcessing) {
    ...
    const auto& cameraTransform = _registry->getComponent<TransformComponent>(*_registry->getEntity<MainCameraTag>());
    const auto& cameraTransformMatrix = TransformComponent::getGlobalTransformMatrix(*_registry->getEntity<MainCameraTag>(), _registry);
...
template <typename MainCameraTag>
void RenderingSystem<MainCameraTag>::setTexture(Material const* material, const std::string& textureName, auto textureMember) {
    UseShader modelShaderUsage(_modelShaderProgram);
