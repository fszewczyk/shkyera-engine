...
class AssetSelector {
 public:
  ...
  void setUpdateCallback(std::function<void(AssetHandle file)> callback) { _updateCallback = std::move(callback); }

  void setClearCallback(std::function<void()> callback) { _clearCallback = std::move(callback); }
    ...
    if (_asset.has_value() && _registry) {
      ...
      if (_registry->hasComponent<NameComponent>(*_asset)) {
        ...
        if (_registry->hasComponent<AssetComponent<AssetType>>(*_asset)) {
          ...
          if (const auto* assetLoader =
                  dynamic_cast<utils::assets::PathAssetLoader<AssetType>*>(assetComponent.constructionFunction.get())) {
...
class TextureAssetSelector : public AssetSelector<Texture> {
 public:
  ...
  ~TextureAssetSelector() { clearAsset(); }
 ...
 private:
  ...
  void clearAsset() override { _texture.reset(); }
