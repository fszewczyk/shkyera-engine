...
PropertiesWidget::PropertiesWidget(std::shared_ptr<Registry> registry)
    : Widget("Properties"), _registry(registry) {}
...
void CameraPropertiesWidget::updateComponents() {
    static auto onlyOnce = [this]() {
        SHKYERA_ASSERT(_registry->getEntity<SceneCamera>().has_value(), "SceneCamera is not registered. Cannot use CameraPropertiesWidget");
...
EnvironmentPropertiesWidget::EnvironmentPropertiesWidget(std::shared_ptr<Registry> registry)
    ...
    SHKYERA_ASSERT(registry->getEntity<Environment>().has_value(), "Environment component is not registered. Cannot create an EnvironmentPropertiesWidget");
}
