...
class FilesystemWidget : public Widget {
 ...
 private:
  ...
  /**
     ...
     */
  static std::string getDisplayableName(std::string name,
                                        size_t maxCharactersInLine = 12);

  std::shared_ptr<Directory>
      _currentDirectory;  ///< The current working directory to display.
  std::vector<std::shared_ptr<Directory>>
      _directoriesToDisplay;  ///< Directories to display in the UI.
  std::vector<std::shared_ptr<File>>
      _filesToDisplay;  ///< Files to display in the UI.

  bool
      _hoveredIcon;  ///< A flag indicating whether an icon is currently hovered.

  static constexpr float CONTENTS_ICON_SIZE =
      64;  ///< The size of icons representing directory contents.
