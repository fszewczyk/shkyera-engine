...
void SceneWidget::drawRuntime() const {
  ...
  ImGui::BeginDisabled();
  ImGui::ImageButton((ImTextureID)Image::ICON_BUTTON_PLAY.getTextureId(),
                     ImVec2(16, 16));
  ImGui::EndDisabled();

  ImGui::SameLine();

  if (ImGui::ImageButton((ImTextureID)Image::ICON_BUTTON_STOP.getTextureId(),
                         ImVec2(16, 16))) {}
}

void SceneWidget::drawScene() {
  if (ImGui::ImageButton((ImTextureID)Image::ICON_BUTTON_PLAY.getTextureId(),
                         ImVec2(16, 16))) {
    adjustSize();
  }
  ImGui::SameLine();
  ...
  ImGui::BeginDisabled();
  ImGui::ImageButton((ImTextureID)Image::ICON_BUTTON_STOP.getTextureId(),
                     ImVec2(16, 16));
  ImGui::EndDisabled();
}
