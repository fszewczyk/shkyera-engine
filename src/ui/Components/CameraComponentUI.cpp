...
CameraComponentUI::CameraComponentUI(CameraComponent* cameraComponent)
  ...
  _fovSlider.setUpdateCallback([this](float fov) { _cameraComponent->fov = fov; });
  _projectionSelector.setUpdateCallback(
      [this](const auto projection) { _cameraComponent->projectionType = projection; });
}
