...
ModelComponentUI::ModelComponentUI(std::shared_ptr<Registry> registry, ModelComponent* modelComponent)
  ...
  _meshSelector.setClearCallback([this]() { _modelComponent->mesh = HandleAndAsset<Mesh>{std::nullopt, nullptr}; });
  ...
  _materialSelector.setClearCallback(
      [this]() { _modelComponent->material = HandleAndAsset<Material>{std::nullopt, nullptr}; });
}
