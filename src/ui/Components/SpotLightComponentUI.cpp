...
SpotLightComponentUI::SpotLightComponentUI(SpotLightComponent* pointLightComponent)
  ...
  _colorSelector.setUpdateCallback([this](const glm::vec3& color) { _spotLightComponent->color = color; });
  _intensitySlider.setUpdateCallback([this](float intensity) { _spotLightComponent->intensity = intensity; });
  _rangeSlider.setUpdateCallback([this](float range) { _spotLightComponent->range = range; });
  _innerCutoffSlider.setUpdateCallback([this](float innerCutoffInDegrees) {
    _spotLightComponent->innerCutoff =
        std::min(glm::radians(innerCutoffInDegrees), 0.999f * _spotLightComponent->outerCutoff);
  });
  _outerCutoffSlider.setUpdateCallback([this](float outerCutoffInDegrees) {
    _spotLightComponent->outerCutoff =
        std::max(glm::radians(outerCutoffInDegrees), 1.001f * _spotLightComponent->innerCutoff);
  });
}
