...
PostProcessingVolumeComponentUI::PostProcessingVolumeComponentUI(
    PostProcessingVolumeComponent* postProcessingVolumeComponent)
  ...
  _antiAliasingSelector.setUpdateCallback(
      [this](bool enabled) { _postProcessingVolumeComponent->antiAliasing = enabled; });
  _toneMappingSelector.setUpdateCallback(
      [this](bool enabled) { _postProcessingVolumeComponent->toneMapping = enabled; });
  _globalSelector.setUpdateCallback([this](bool enabled) { _postProcessingVolumeComponent->global = enabled; });
  _bloomThresholdSlider.setUpdateCallback(
      [this](float threshold) { _postProcessingVolumeComponent->bloomThreshold = threshold; });
  _bloomWeightSlider.setUpdateCallback([this](float weight) { _postProcessingVolumeComponent->bloomWeight = weight; });
  _gammaSlider.setUpdateCallback([this](float gamma) { _postProcessingVolumeComponent->gamma = gamma; });
  _ssaoStrengthSlider.setUpdateCallback(
      [this](float strength) { _postProcessingVolumeComponent->ssaoStrength = strength; });
  _ssaoRadiusSlider.setUpdateCallback([this](float radius) { _postProcessingVolumeComponent->ssaoRadius = radius; });
}
