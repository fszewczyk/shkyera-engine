...
BillboardComponentUI::BillboardComponentUI(std::shared_ptr<Registry> registry, BillboardComponent<>* billboardComponent)
    : _billboardComponent(billboardComponent),
      ...
      _scaleSelector(
          "Scale", _billboardComponent->scale,
          {{BillboardComponent<>::Scale::Camera, "Camera Fixed"}, {BillboardComponent<>::Scale::World, "World Fixed"}}),
  ...
  _materialSelector.setClearCallback(
      [this]() { _billboardComponent->material = HandleAndAsset<Material>{std::nullopt, nullptr}; });

  auto setUpdateCallback = [this](auto& selector, auto billboardProperty) mutable {
    selector.setUpdateCallback(
        [this, billboardProperty](const auto& val) { (_billboardComponent->*billboardProperty) = val; });
  };
