...
AmbientLightComponentUI::AmbientLightComponentUI(AmbientLightComponent* ambientLightComponent)
  ...
  _colorSelector.setUpdateCallback([this](const glm::vec3& color) { _ambientLightComponent->color = color; });
  _intensitySlider.setUpdateCallback([this](float value) { _ambientLightComponent->intensity = value; });
}
