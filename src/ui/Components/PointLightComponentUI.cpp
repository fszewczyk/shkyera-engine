...
void PointLightComponentUI::draw() {
  ...
  if (ImGui::TreeNodeEx("Point Light", ImGuiTreeNodeFlags_DefaultOpen)) {
    glm::vec3& ambient = _pointLightComponent->ambient;
    ...
    ImGui::Text("Ambient");
    ImGui::SameLine(120);
    ImGui::DragFloat("R##Ambient", &ambient[0], 0.01f, 0.0f, 1.0f);
    ImGui::SameLine();
    ImGui::DragFloat("G##Ambient", &ambient[1], 0.01f, 0.0f, 1.0f);
    ImGui::SameLine();
    ImGui::DragFloat("B##Ambient", &ambient[2], 0.01f, 0.0f, 1.0f);
