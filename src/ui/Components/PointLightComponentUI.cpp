...
PointLightComponentUI::PointLightComponentUI(PointLightComponent* pointLightComponent)
  ...
  _colorSelector.setUpdateCallback([this](const glm::vec3& color) { _pointLightComponent->color = color; });
  _intensitySlider.setUpdateCallback([this](float intensity) { _pointLightComponent->intensity = intensity; });
  _rangeSlider.setUpdateCallback([this](float range) { _pointLightComponent->range = range; });
}
