...
UI::UI(std::shared_ptr<Registry> registry)
    : _open(true), _registry(std::move(registry)) {
    initialize();
}
...
void UI::beginFrame() {
    ...
    if (firstTime) {
        ...
        ImGuiID dock_id_right, dock_id_left, dock_id_left_bottom, dock_id_left_up,
            dock_id_left_up_left, dock_id_left_up_right;
        ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Left, 0.75f,
                                    &dock_id_left, &dock_id_right);
        ImGui::DockBuilderSplitNode(dock_id_left, ImGuiDir_Down, 0.3f,
                                    &dock_id_left_bottom, &dock_id_left_up);
        ImGui::DockBuilderSplitNode(dock_id_left_up, ImGuiDir_Left, 0.3f,
                                    &dock_id_left_up_left, &dock_id_left_up_right);
