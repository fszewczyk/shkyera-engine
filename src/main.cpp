...
static shkyera::Entity addModel(std::shared_ptr<shkyera::Registry> registry,
                                ...
                                shkyera::HandleAndAsset<shkyera::Mesh> mesh) {
    ...
    registry->getComponent<ModelComponent>(entity).material = utils::assets::add<Material>(registry.get(), std::make_unique<utils::assets::MaterialLoader>(registry.get(), Material{}));
    registry->addComponent<BoxColliderComponent<RuntimeMode::DEVELOPMENT>>(entity);
    registry->getComponent<BoxColliderComponent<RuntimeMode::DEVELOPMENT>>(entity).box = std::get<AssetRef<Mesh>>(mesh)->getBoundingBox();
...
void loadScene(std::shared_ptr<shkyera::Registry> registry) {
    ...
    auto cylinderModel = addModel(registry, {0, 0, 0}, "Cylinder Model", utils::assets::add<Mesh>(registry.get(), Mesh::Factory::Type::CYLINDER));
    auto cylinderWireframe = addWireframe(registry, {0, 3, 0}, "Cylinder Wireframe", utils::assets::add<Wireframe>(registry.get(), Wireframe::Factory::Type::CYLINDER));
    ...
    addWireframe(registry, {3, 3, 0}, "Cube Wireframe", utils::assets::add<Wireframe>(registry.get(), Wireframe::Factory::Type::CUBE));

    // Add Sphere and its wireframe
    addModel(registry, {3, 6, 3}, "Sphere", utils::assets::add<Mesh>(registry.get(), Mesh::Factory::Type::SPHERE));
    addWireframe(registry, {3, 9, 3}, "Sphere Wireframe", utils::assets::add<Wireframe>(registry.get(), Wireframe::Factory::Type::SPHERE));

    // Add World Plane
    auto bunny = addModel(registry, {0, -1, 0}, "Bunny", utils::assets::add<Mesh>(registry.get(), "resources/models/bunny.obj"));
    registry->getComponent<TransformComponent>(bunny).setScale({15, 15, 15});

    auto worldPlane = addModel(registry, {0, -2, 0}, "Plane", utils::assets::add<Mesh>(registry.get(), Mesh::Factory::Type::PLANE));
    ...
    registry->getComponent<SkyboxComponent>(environment).skyboxCubeMap = utils::assets::add<CubeMap>(registry.get(), std::make_unique<utils::assets::CubeMapLoader>(registry.get(), std::array{skyboxUp, skyboxDown, skyboxLeft, skyboxRight, skyboxFront, skyboxBack}));

    Material mat{};
    mat.albedoTexture = utils::assets::add<Texture>(registry.get(), "resources/skyboxes/day/py.png");
    fireplaceEmitter.material = utils::assets::add<Material>(registry.get(), std::make_unique<utils::assets::MaterialLoader>(registry.get(), mat));
    registry->addComponent<NameComponent>(*std::get<OptionalAssetHandle>(fireplaceEmitter.material)).setName("Hardloaded Material");
    registry->getHierarchy().attributeChild(*registry->getEntity<AssetRoot>(), *std::get<OptionalAssetHandle>(fireplaceEmitter.material));
}
