...
class MaterialLoader : public AssetLoader<Material> {
   public:
    explicit MaterialLoader(Registry* registry, Material mat_)
        : _registry(registry), mat(std::move(mat_)) {
...
class CubeMapLoader : public AssetLoader<CubeMap> {
   public:
    ...
    CubeMapLoader(Registry* registry, Faces faces_)
        : _registry(registry), faces(std::move(faces_)) {
