...
Texture::Texture(const std::filesystem::path& path) : Texture(utils::assets::read<Image>(path)) {}
...
Texture::Texture(Texture&& other) noexcept
    : _textureID(std::exchange(other._textureID, 0)), _size(std::exchange(other._size, glm::vec2{0})) {}
...
void Texture::setData(GLenum internalFormat, uint32_t width, uint32_t height, GLenum format, GLenum type,
                      const void* data) {
  bind();
