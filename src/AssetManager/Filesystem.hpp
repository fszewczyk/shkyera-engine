...
class Directory {
 ...
 private:
  ...
  std::vector<std::shared_ptr<Directory>>
      _directories;  ///< List of subdirectories in the directory.

  static std::shared_ptr<Directory>
      _rootDirectory;  ///< The root directory of the filesystem.
  static std::map<std::filesystem::path, std::shared_ptr<File>>
      _mapFiles;  ///< Map of file paths to file objects.
};
