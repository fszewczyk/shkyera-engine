...
void Mesh::loadFromFile(const std::filesystem::path& filepath) {
  ...
  size_t numberOfVertices = std::accumulate(
      shapes.begin(), shapes.end(), 0,
      [](size_t accumulatedSize, const auto& shape) { return accumulatedSize + shape.mesh.indices.size(); });
  ...
  for (const auto& shape : shapes) {
    for (const auto& index : shape.mesh.indices) {
      glm::vec3 position(attrib.vertices[3 * index.vertex_index + 0], attrib.vertices[3 * index.vertex_index + 1],
                         attrib.vertices[3 * index.vertex_index + 2]);

      glm::vec3 normal(0.0f, 0.0f, 0.0f);
      if (index.normal_index >= 0) {
        normal = glm::vec3(attrib.normals[3 * index.normal_index + 0], attrib.normals[3 * index.normal_index + 1],
      ...
      if (index.texcoord_index >= 0) {
        texcoord =
            glm::vec2(attrib.texcoords[2 * index.texcoord_index + 0], attrib.texcoords[2 * index.texcoord_index + 1]);
      }
...
Mesh Mesh::Factory::createTorus(float innerRadius, float outerRadius, int radialSegments, int tubularSegments) {
  ...
  for (int i = 0; i <= radialSegments; ++i) {
    ...
    for (int j = 0; j <= tubularSegments; ++j) {
      ...
      glm::vec3 position =
          center + glm::vec3(tubeRadius * cosTheta * cosPhi, tubeRadius * sinTheta * cosPhi, tubeRadius * sinPhi);
